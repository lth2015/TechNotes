...
下面的图是应用发布的时序图，它展示了用户的应用发布操作在容器引擎平台中是如何工作的，用户进行应用发布后，容器引擎平台的Web端会给后台一个Post请求，后台收到请求后会进行一些验证操作，然后它会给Kubernetes的APIServer发送一个Post请求，Kubernetes收到请求后会创建应用实例，后台在给Kubernetes发送请求的同时也会在MySQL中添加一条操作记录，等两个操作都成功返回后，界面上会给用户提示发布应用成功。


* 容器引擎平台Overview

这张图是容器引擎平台的全局概览图，它展示了应用发布的过程以及在多个环境中的传递。最上面的一层展示了多个环境，也说明各个环境的发布顺序即：先是QA环境、
再到生产内测、再到生产环境；中间那层说明了使用关键组件:Gitlab,Jenkins,Kubernetes；最下面的层展示了镜像的同步过程，在QA环境中有很少的镜像会发布到生产环境中。

* 容器引擎平台界面

这张是我们容器引擎平台的操作界面的应用管理部分，在这个页面上，用户可以搜索自己的应用，可以查看应用实例的状态、所属的物理机、应用实例的详情等，还可以进行升级、回滚、扩容、删除操作等容器的生命周期管理工作。

* 监控模块

监控管理主要是想把计算资源的使用情况实时展示给用户，在资源不足的情况下能够给管理员报警，另外应用的监控我们已经在别的项目中实现了，就不在容器引擎平台中做了。监控主要涉及三个部分：计算节点监控、容器的资源使用率、数据中心监控。我们在做监控的同时也做了一部分可视化的工作，我们能够实时显示每个应用的实例个数，以及显示整个容器引擎的拓扑图。

* 监控模块的Dashboard

我们在容器引擎平台上实现了一个Dashboard，让每个组织的管理员实时查看他们购买的资源使用情况，上面的四个饼图展示了他们在多个数据中心购买的计算资源的使用情况，第一个饼图是三个数据中心资源使用的平均情况；下面的图展示了最近一周操作统计，按照不同的操作类型进行统计，可以让管理员快速统计最近一周的主要部署操作。
...
在容器引擎平台的实现过程中，我们也遇到了一些问题，我这里列举了其中重要的几个。第一个是如何快速、简便的调用容器引擎的API，快速开发容器引擎平台，这个之所以是难点，是因为我们的团队人员不足，再加上开发进度也很紧张，我们需要找到一条快速可行的办法又不为后面的迭代挖坑的办法。第二个是容器的镜像构建时，代码的版本与镜像的版本如何同步的问题，我们在线上查找问题时，特别是不太好复现的问题，我们需要找到这个镜像对应的代码版本，我们之后在自动构建时把代码的Commit版本号写在了镜像中，这样镜像就跟代码的版本建立了关系，就能对应起来了。第三个问题是容器引擎使用的都是最新的技术，合理的控制技术风险和项目风险是一个严峻的挑战，对于一项新技术，新技术的背后的思想让大多数技术人员认可和接受需要时间，另外新技术落地不仅仅是技术问题，还需要各个方面的配合。最后的一个问题是自动扩容的实现，对于应用开发而言，什么场景下触发自动扩容是个非常困难的问题，很难有一个统一的标准，经过跟应用方多次探讨和论证，一致认为先实现手工扩容方式，再实现自动扩容的方式。最后我们找到了一个通用的解决办法，即基于CPU使用率的自动扩容算法。

* 算法描述

计算扩容后的实例数的公式如下：，Ceil为取整操作，表示大于或等于某数最近的一个整数，通常我们称之为“取底“操作。Sum是算数求和操作，CurrentPodsCPUUtilization为最近一分钟内某个实例的CPU使用率的平均值，Target是我们人为设置的CPU使用上限。CPU利用率是指最近的Pod使用量（最近一分钟的平均值，从heapster中获得）除以设定的每个Pod的CPU使用率限额，该算法每隔30秒进行一次自动扩容判断，同时还要设置冷却时间，因为每次启动、停止Pod会产生CPU噪声干扰，自动扩容后要等待一段时间（即冷却时间），以便让扩容的触发更加准确。


* 跨多个网络的镜像同步

我们生产环境部署在三个不同的数据中心，三个数据中心通过网络专线联通，为了避免同时向三个数据中心传输镜像的带宽消耗（需要新拉专线），我们设计了这种方案，先将镜像推送到数据中心A，数据中心B和C从A处同步镜像。
...
2017.08 – 2017.09：完成剩余功能模块开发

2017.09 – 2017.10：完成平台系统测试，并正式投入使用

2017.10 – 2017.12 ：完成论文写作

2018.01 – 2018.02 ：完成论文答辩 
